...
library(ggplot2)
melted = melt(my_df)
lab = c(names(which.max(my_df["1",])),names(which.max(my_df["30",])))
where = c(max(my_df["1",]),max(my_df["30",]))
ggplot(melted, aes(factor(Var1), value))+geom_boxplot()+labs(title = "Number of Instances Per Activity For Each Subject", x="subject", y="count")+geom_text(aes("1",where[1]+2), label=lab[1], color="blue")+geom_text(aes("30",where[2]+2), label=lab[2], color="blue")
...
for (i in 1:length(model_list)){
  accuracy[i,] = as.numeric(model_list[[i]]@model$cross_validation_metrics_summary['accuracy',c('mean','sd')])
}
